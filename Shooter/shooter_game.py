...
class Asteroid(Hero):
    ...
    def update(self):
        ...
        if self.direction == 0:
            ...
            self.rect.x -= self.speed
        if self.rect.y > win_width or self.rect.x > win_width or self.rect.x < 0:
            self.rect.x = randint(80, win_width - 80)
            self.rect.y = 0
...
for i in range(1, 4):
    asteroid = Asteroid(img_ast, randint(80, win_width - 80), -40, 80, 50, randint(1,3), randint(0,1))
    asteroids.add(asteroid)
bonus_hp = Hero('hp.png', randint(80, win_width - 80), randint(80, win_height-120), 30, 30, 0)
...
while run:
    ...
    if not finish:
        ...
        if sprite.spritecollide(bonus_hp, bullets, True):
            ...
            bonus_hp.rect.y = randint(80, win_height - 120)
        ...
        text_lose = font2.render("Пропеущено: " + str(lost), 1, (255, 255, 255))
